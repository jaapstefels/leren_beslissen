...
def convert_and_resize(base_path='mass_roads_test', new_base_path = 'mass_roads_new'
):
	...
	train_mask_files = glob(os.path.join(train_mask, "*.tiff"))
	train_mask_ids = [s[len(train_mask)+1:-5] for s in train_mask_files]
	valid_mask_files = glob(os.path.join(valid_mask, "*.tiff"))
	valid_mask_ids = [s[len(valid_mask)+1:-5] for s in valid_mask_files]
	test_mask_files = glob(os.path.join(test_mask, "*.tiff"))
